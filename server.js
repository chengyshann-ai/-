...
function buildPrompt(b) {
  ...
  let p = '你是欧洲旅行专家，熟悉小红书推荐、Google Maps高分景点、当地特色市场。输出纯JSON，不要Markdown。\n\n';
  p += '参数: ' + b.days + '天 ' + (lm[b.level]||'') + ' ' + b.departure + '→' + (b.cities||[]).join('→') + ' ' + b.startDate + '\n\n';
  p += '【景点】\n';
  p += '1. 每天2-3个景点，格式: "English Name 中文名"（先英文后中文，中间空格）\n';
  p += '2. 除了地标，加入: 小红书热门打卡点、Google高分餐馆、公园、市场、美食街、河边步道\n';
  ...
  p += '\n返回纯JSON:{"route":"...","days":[{"day":1,"date":"YYYY-MM-DD","city":"城市,国家","touringSpots":["Eiffel 埃菲尔铁塔"],"accommodation":"住宿","transportation":"交通"}]}';
  return p;
}
// ===== Qwen =====
