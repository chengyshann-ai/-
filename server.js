...
function buildPrompt(b) {
  ...
  let p = '你是欧洲深度旅行专家，比任何攻略书都懂当地。输出纯JSON，不要Markdown。\n\n';
  p += '参数: ' + b.days + '天 ' + (lm[b.level]||'') + ' ' + b.departure + '→' + (b.cities||[]).join('→') + ' ' + b.startDate + '\n\n';
  p += '【景点·必须每次不同·超越固定列表】\n';
  p += '你必须推荐以下类型的景点，不能只用地标:\n';
  p += '1. 小红书/抖音热门: 网红咖啡馆、打卡餐厅、拍照街区、小众美术馆\n';
  p += '2. 特色购物: 中古/ vintage市场、独立书店、买手店、本地市集\n';
  p += '3. 城市漫步: 最美街道、河边步道、涂鸦区、设计师街区\n';
  p += '4. 短时景点: 咖啡馆(1h)、书店(1h)、教堂外观(30min) → 可多排1-2个\n';
  p += '5. 一日游: 城市周边可当天往返的热门路线(如佛罗伦萨→比萨/锡耶纳)\n';
  p += '6. 每天2-4个景点混合: 1个地标+1个隐藏宝藏+1个休闲点(咖啡馆/书店/市集)\n\n';
  p += '【景点格式】"English Name 中文名"（先英文后中文）\n';
  p += '【铁律】同城多天景点不重复；周一闭馆不排；换乘日1个；到达日不排需门票的\n\n';
  p += '【交通格式】\n';
  ...
  p += '【住宿】"X晚\\n酒店品牌\\n地址: 街道, 城市"\n\n';
  p += '【禁止】不加距离数字、不加攻略解释、不加"周一闭馆"括号说明；不要和参考行程的景点重复';
  if (b.localItinerary && b.localItinerary.days) {
    p += '\n参考行程(请替换其中至少50%的景点为新的隐藏宝藏):\n' + JSON.stringify(b.localItinerary) + '\n';
  }
  p += '\n返回纯JSON:{"route":"...","days":[{"day":1,"date":"YYYY-MM-DD","city":"城市,国家","touringSpots":["Eiffel 埃菲尔铁塔"],"accommodation":"...","transportation":"..."}]}';
  return p;
}
// ===== Qwen =====
